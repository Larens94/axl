...
def _render_node(node: UiNode) -> str:
    ...
    if node.component_id == 4:
        ...
        size = max(1, min(2, int(_value(node, 4, 1))))
        action = node.events[0].action_id if node.events else node.node_id
        return f'<button class="ax-card ax-tone-{tone} ax-size-{size}" data-action="{action}"><span class="ax-rank">{node.node_id}</span><strong>{html.escape(str(_value(node, 1)))}</strong><small>{html.escape(str(_value(node, 2)))}</small><i>▶</i></button>'
...
_CSS += ".ax-size-2{grid-column:span 2;min-height:190px}"
