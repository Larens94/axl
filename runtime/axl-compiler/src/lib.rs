...
(cd "$ROOT/backend" && cargo run) & BACKEND_PID=$!
...
echo "AXL smoke test passed: auth guard, JWT, seeded data and query pagination"
"#;
...
"#).unwrap();
        ...
        assert!(frontend.contains("ThemedLayoutV2"));
