...
| Runtime | records, transforms, `parallel`, `race`, retry/timeout, flow/capacity calls, typed `emit`/subscriptions, jobs (`enqueue`/`tick`), `Result` propagation, forkable configured provider ABI and HTTP | state; full Gate 4 UI kit (filter + pagination + **drawer** started) |
...
Gate 4 has started: `ui` / `page` / `form` / **`drawer`** nodes lower to Graph IR, emit
`axl-ui/1`, and `render` / `render_form` / drawer overlay evaluate bound flows into
HTML with a navigation shell; `serve` returns `text/html` on GET for page, form and
drawer paths (`balance-ui.axl`, `form-demo.axl`, `drawer-boundary.axl`). Component
registry, mobile bottom nav, modal and KPI/charts remain open.
