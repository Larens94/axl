...
|---|---|---|
| Source | multiline AXL, enum match, map/filter/sort/fold and flows with `make`, `run`, `call`, `return` | branch blocks, collection literals/grouping and async |
...
| Runtime | records, map/filter/sort/fold, flow/capacity calls, `Result` propagation, provider ABI and HTTP | grouping, state, events, durable persistence and UI |
...
next backend milestones; grouping and async providers remain language
and runtime gates.
