...
| Backend | scalar and composite body/path/query/header/cookie binding, Axum, typed bearer auth, ordered request and response middleware, typed events/subscriptions, capacity-backed jobs and durable SQLite | secrets, JWT/OAuth, cache and observability |
...
Secret references, JWT/OAuth providers, cache and observability are the next
backend milestones.
