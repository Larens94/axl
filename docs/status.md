...
| Runtime | records, transforms, `parallel`, `race`, retry/timeout, flow/capacity calls, typed `emit`/subscriptions, jobs (`enqueue`/`tick`), `Result` propagation, forkable configured provider ABI and HTTP | state; full Gate 4 UI kit (filter + pagination + drawer + **modal** started) |
...
HTML with a navigation shell; `serve` returns `text/html` on GET for page, form, drawer and
modal paths (`balance-ui.axl`, `form-demo.axl`, `drawer-boundary.axl`, `modal-boundary.axl`). Component
registry, mobile bottom nav and KPI/charts remain open.
