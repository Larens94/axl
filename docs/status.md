...
| Runtime | records, transforms, `parallel`, `race`, retry/timeout, flow/capacity calls, typed `emit`/subscriptions, jobs (`enqueue`/`tick`), `Result` propagation, forkable configured provider ABI and HTTP | state; full Gate 4 UI kit (filter + pagination + drawer + modal + **bottom nav**; KPI/charts open) |
...
Gate 4 has started: `ui` / `page` / `form` / **`drawer`** / **`modal`** nodes lower to Graph IR, emit
`axl-ui/1`, and `render` / `render_form` / drawer / modal overlays evaluate bound flows into
HTML with a navigation shell (desktop sidebar + **mobile bottom nav**); `serve` returns `text/html`
on GET for page, form, drawer and modal paths (`balance-ui.axl`, `form-demo.axl`,
`drawer-boundary.axl`, `modal-boundary.axl`, `bottom-nav-boundary.axl`). Component
registry slots and KPI/charts remain open.
